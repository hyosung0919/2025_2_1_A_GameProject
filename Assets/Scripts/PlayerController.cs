    ...
    void CheckGrounded()
    {
        ...
        if (isGrounded && velocity.y < 0)
        {
            ...
            if (!wasGrounded && animator != null)
            {
                ...
                Debug.Log("착지");
                EffectManager.instance.PlayeEffect("착지이펙트" , transform.position);
            }
        }
    }
    ...
    void HandleAttack()
    {
        ...
        if(Input.GetKeyDown(KeyCode.Alpha1)&& !isAttacking)         //공격 중이 아닐때 키를 누르면 공격
        {
            ...
            if(animator != null)
            {
                ...
                EffectManager.instance.PlayeEffectWithDelay("공격이펙트", effectPos.transform.position, Quaternion.identity, 0.5f, 1.0f);
            }
        }
    }
